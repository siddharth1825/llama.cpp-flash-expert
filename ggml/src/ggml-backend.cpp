...
// Custom split execution: same input-copy logic as compute_splits, but calls
// a user callback per split instead of graph_compute_async. This lets the
// caller control synchronization and defer GPU work for pipeline parallelism.
enum ggml_status ggml_backend_sched_compute_splits_custom(
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        // ── Pre-copy callback: lets caller collect deferred results ──
        {
            enum ggml_status ec = cb(split_id, split_backend, &split->graph, /*pre_copy=*/true, user_data);
            if (ec != GGML_STATUS_SUCCESS) {
                return ec;
            }
        }

        // ── Copy input tensors (identical to compute_splits) ──
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ggml_backend_t input_backend = ggml_backend_sched_get_tensor_backend(sched, split->inputs[input_id]);
            struct ggml_tensor * input = split->inputs[input_id];
            struct ggml_tensor * input_cpy = tensor_copy(input, split_backend_id, sched->cur_copy);
        ...
        // ── Call user callback instead of graph_compute_async ──
        enum ggml_status ec = cb(split_id, split_backend, &split->graph, /*pre_copy=*/false, user_data);
        if (ec != GGML_STATUS_SUCCESS) {
            return ec;
        }
